<?php
    ...
    class Account_Ledger extends Admin_Controller
    {
        ...
        function index()
        {
            ...
            $query            = "SELECT i.id, type_c 'type', i.date, i.`type_i` 'invoice_type', c.`name`, i.`id` 'invoice_no', i.`reference_no`, IF(i.`type_i` = 3, sum(id.qty), '') 'make_sheet', IF(i.`type_i` = 2, sum(id.qty), '') 'sale_sheet', i.client_id, IF(i.`type_i` = 1 OR i.`type_i` = 3, FORMAT(i.total_amount, 2), '') 'credit', IF( i.`type_i` = 2, FORMAT(i.total_amount, 2), '') 'debit', 'balance' FROM invoice i INNER JOIN invoice_detail id ON i.id = id.invoice_id INNER JOIN clients c ON i.`client_id` = c.`id` WHERE i.`status` = '1' AND id.`status` = '1' $where GROUP BY i.id ORDER BY i.`date`, i.`created_at`";
