<?php
    ...
    class Customer_Invoice extends Admin_Controller
    {
        ...
        function view_()
        {
            ...
            if($type_i == '2')
            {
                $query1 = "SELECT i.`id`, i.`type_i`, i.`type_c`, i.reference_no, i.`date`, c.`name`, i.`total_amount`, i.`status` FROM " . $this->tableInvoice . " i INNER JOIN " . $this->tableClients . " c ON i.`client_id` = c.`id`  WHERE i.`status` != '-1' AND i.`type_c` = '" . $this->type_c . "' AND i.`id` = '" . $id . "'";
