<?php
    ...
    class Vendor_Invoice extends Admin_Controller
    {
        var $controller = 'vendor_invoice';
        var $table = 'stock';
        ...
        function new_()
        {
            ...
            if($this->data['type_i'] == '3')
            {
                echo $this->loadHtml('vendor_invoice_view', $this->data);
            }
            else if($this->data['type_i'] == '4')
            {
                echo $this->loadHtml('vendor_payment_invoice_view', $this->data);
            }
            else
            {
        ...
        function save_()
        {
            ...
            $invoiceId = $this->insert($this->table_, $insert);
            ...
            if($invoiceType == '3')
            {
                ...
                for ($a = 0; $a < count($product); $a++)
                {
                    ...
                    $invoiceDetailId = $this->insert($this->table2_, $insert2);
            ...
            else if($invoiceType == '4')
            {
                ...
                for ($a = 0; $a < count($priceArray); $a++)
                {
                    ...
                    $this->insert($this->table2_, $insert2);
                }
            }
        ...
        function view_()
        {
            ...
            if($type_i == '3')
            {
                $query1 = "SELECT i.`id`, i.`type_i`, i.`type_c`, i.reference_no, i.`date`, c.`name`, i.`total_amount`, i.`status` FROM " . $this->table_ . " i INNER JOIN " . $this->table3 . " c ON i.`client_id` = c.`id`  WHERE i.`status` != '-1' AND i.`type_c` = '" . $this->type_c . "' AND i.`id` = '" . $id . "'";
                $this->data['data']             = $this->executeQuery($query1)->row();

                $query2  = "SELECT p.id, p.`name` 'product', id.`qty` 'quantity', id.`rate`, id.`price` FROM " . $this->table2_ . " id INNER JOIN " . $this->table4_ . " p ON id.`product_id` = p.`id` WHERE id.`invoice_id` = '" . $id . "' AND id.`status` != '-1' GROUP BY id.`id`";
                $this->data['data2']['data']    = $this->executeQuery($query2)->result();
                $this->data['data2']['colunms'] = array('product', 'quantity', 'rate', 'price');

                $query3 =  "SELECT ii.`productId`, i.`name`, pd.`quantity`, id.qty, (pd.`quantity`*id.qty) 'quantity', TRUNCATE(((pd.`quantity`*id.qty)/i.qty_per_bag),2) 'total_bags' FROM invoice_items ii INNER JOIN products p ON ii.`productId` = p.`id` INNER JOIN `product_detail` pd ON p.`id` = pd.`product_id` INNER JOIN items i ON ii.`itemId` = i.`id` INNER JOIN invoice_detail id ON id.`id` = ii.invoiceDetailId WHERE ii.`invoiceId` = '" . $id ."' AND ii.`status` = '1' AND pd.`status` = '1' GROUP BY pd.`id`";
                ...
                $this->data['data3']['colunms'] = array('name', 'quantity', 'total_bags');
            }
            else if($type_i == '4')
            {
                $query1 = "SELECT i.`id`, i.`type_i`, i.`type_c`, i.reference_no, i.`date`, c.`name`, i.`total_amount`, i.`status` FROM ".$this->table_." i INNER JOIN ".$this->table3." c ON i.`client_id` = c.`id`  WHERE i.`status` != '-1' AND i.`type_c` = '".$this->type_c."' AND i.`id` = '".$id."'";
                ...
                $query2   = "SELECT i.`price`, i.`desc` 'description' FROM ".$this->table2_." i WHERE i.`invoice_id` = '" . $id . "' AND i.`status` != '-1' GROUP BY i.`id`";
                $this->data['data2']['data']    = $this->executeQuery($query2)->result();
        ...
        function edit_()
        {
            ...
            $this->data['data']       = $this->select('', $this->table_, "id = " . $id)->row();
            $this->data['data2']      = $this->select('', $this->table2_, "invoice_id = " . $id . " AND `status` != '-1'")->result();
        ...
        function update_()
        {
            ...
            if ($this->update($this->table_, $update, 'id', $invoiceId))
            {
                $this->actionResponse($this->input->post('id'), 2);
            }

            if ($this->update($this->table2_, array('status' => '-1'), 'invoice_id', $invoiceId))
            {
                if ($type_i == 3)
                {
        ...
        function delete_()
        {
            ...
            echo $this->update($this->table_, $update, 'id', $id);
            ...
            if(count($data) > 0)
            {
                ...
                for($a = 0 ; $a < count($data) ; $a++)
                {
                    ...
                    <tr class="product-contain-items-<?=$rowNo?>">
                        <td>
                             <span style="width: 100px; float: left"> Qty ( <?=ucfirst($data[$a]->itemType)?> ) </span>
                        ...
                        <td style="width: 270px;">
                            ...
                            <select name="item-<?=$rowNo?>[]" id="item-<?=$rowNo?>-<?=$a?>" onchange="getTotalBags(this.value, '<?=$rowNo?>', '<?=$a?>')" class="[c-e] chosen">
                                <?= $this->selectBox_db('id', 'name', $itemsSelectBox) ?>
                            </select>
                            <span id="loading-<?=$rowNo?>-<?=$a?>" style="width: 20px"></span>
                        </td>
                        <td>
                            <span>Bags : </span>
                            <input type="hidden" name="qty-per-bag-<?=$rowNo?>[]" id="qty-per-bag-<?=$rowNo?>-<?=$a?>" value="" />
                            <input type="text" name="total-bags-<?=$rowNo?>[]" id="total-bags-<?=$rowNo?>-<?=$a?>" readonly value="" style="width: 60px"/>
                        </td>
                    </tr>
                    <?
                }
